...
xticks = 250 #max number of races
...
while numRelations < relationshipGoal : # the scoreboard is not complete, need to run more races 
  try :
    ...
    # Keep a record of completed horses
    for horse in horses.copy() :
      if len(scoreboard[horse]) == len(horses) - 1 :
        #horses -= set([horse])
        completed.add(horse)
